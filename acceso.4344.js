(function(){
  ...
  function entrar(){location.replace('main.html?v=4.3.54');}
  ...
  companyForm.addEventListener('submit',async event=>{
    ...
    try{
      ...
      mostrarMensaje('Empresa validada en el Directorio central. La URL real quedó guardada, pero será revalidada antes de cada acceso.','exito',mensajeEmpresa);
  ...
  $('#abrirConfiguracionEmpresa').addEventListener('click',()=>{
    ...
    mostrarMensaje('El Directorio central Supabase está integrado y no es editable. Antes de cada acceso se valida estado y URL real.','exito',$('#mensajeConexionAvanzada'));
  ...
  $('#restaurarDirectorio')?.addEventListener('click',()=>{
    $('#urlDirectorioAvanzada').value=window.CONFIGURACION_FLOTAS.DIRECTORIO_EMPRESAS_URL||'';
    mostrarMensaje('La URL central Supabase ya está integrada y permanece fija.','exito',$('#mensajeConexionAvanzada'));
  });
  $('#formularioConexionAvanzada')?.addEventListener('submit',event=>{
    event.preventDefault();
    mostrarMensaje('La URL del Directorio central no se modifica desde el navegador y se consulta antes de cada acceso.','exito',$('#mensajeConexionAvanzada'));
  });
