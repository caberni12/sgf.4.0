(function () {
  ...
  function direccionDirectorioActual() {
    // El Directorio es un bootstrap fijo integrado en la aplicación Web.
    // Nunca se sustituye por configuración remota ni por datos guardados localmente.
  ...
  function solicitarDirectorioJsonp(rut, claveInstalacion='') {
    ...
    return new Promise((resolve, reject) => {
      ...
      if(String(claveInstalacion||'').trim()) direccion.searchParams.set('claveInstalacion', String(claveInstalacion).trim());
  ...
  async function resolverConexionEmpresa(rutIngresado, claveInstalacion='') {
    ...
    const respuesta = await solicitarDirectorioJsonp(rut, claveInstalacion);
    if (!respuesta || respuesta.ok !== true) throw new Error(String(respuesta?.error || 'EMPRESA_NO_REGISTRADA'));
    const empresaId = String(respuesta.empresa?.idEmpresa || respuesta.empresa?.empresaId || respuesta.empresa?.ID_EMPRESA || '').trim();
    ...
    if (!empresaId || !nombre || !urlHttpsValida(url)) throw new Error('RESPUESTA_DIRECTORIO_INVALIDA');
    await comprobarServicioEmpresa(url);
    const ahoraConexion = new Date().toISOString();
    conexionEmpresa = { empresaId, nombre, rut:rutFormateado, url, guardadaEn:ahoraConexion, validadaEn:ahoraConexion, estadoDirectorio:'ACTIVA' };
    localStorage.setItem(config.CLAVE_CONEXION_EMPRESA, JSON.stringify(conexionEmpresa));
    // Solicita almacenamiento persistente del origen cuando el navegador lo soporta.
    // La conexión real permanece en este computador entre cierres y reinicios.
  ...
  async function validarEmpresaActivaParaAcceso({forzar=false}={}) {
    ...
    if (!rutEmpresaValido(rut) || !urlHttpsValida(conexionEmpresa.url)) {
      throw new Error('CONEXION_EMPRESA_REQUERIDA');
    }

    // Migración excepcional de instalaciones Web antiguas que guardaron URL/RUT
    // pero todavía no tenían empresa_id. Solo en ese caso se consulta Directorio.
    if (!String(conexionEmpresa.empresaId || '').trim()) {
      const respuesta = await solicitarDirectorioJsonp(rut);
      if (!respuesta || respuesta.ok !== true) {
        throw new Error(String(respuesta?.error || 'EMPRESA_NO_REGISTRADA'));
      }
      ...
      const nombre = String(respuesta.empresa?.nombre || conexionEmpresa.nombre || '').trim();
      const rutFormateado = String(respuesta.empresa?.rut || rut).trim();
      const url = String(respuesta.conexion?.url || conexionEmpresa.url || '').trim();
      if (!empresaId || !nombre || !urlHttpsValida(url)) throw new Error('RESPUESTA_DIRECTORIO_INVALIDA');
    ...
    // Uso normal: NO se consulta Apps Script. El navegador usa directamente
    // la URL real persistida en este computador. La autoridad de seguridad
    // continúa siendo flotas-api y la sesión empresarial del servidor.
    return obtenerConexionEmpresa();
  }

  async function sincronizarConexionEmpresa({forzar=false}={}) {
    if (!conexionEmpresa) throw new Error('CONEXION_EMPRESA_REQUERIDA');
    // No consulta el Directorio: reutiliza la URL real persistida.
    return obtenerConexionEmpresa();
  }
  ...
  function prepararSolicitudRemota(accion, carga) {
    ...
    solicitud.empresaId = String(conexionEmpresa?.empresaId || '').trim();
    solicitud.empresaRut = String(conexionEmpresa?.rut || '').trim();
    solicitud.empresaNombre = String(conexionEmpresa?.nombre || '').trim();
