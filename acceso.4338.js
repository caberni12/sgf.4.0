(function(){
  ...
  function entrar(){location.replace('main.html?v=4.3.38');}
  ...
  companyForm.addEventListener('submit',async event=>{
    ...
    try{
      ...
      mostrarMensaje('La URL real quedó guardada en este computador. Se reutilizará aunque cierre el navegador o apague el equipo.','exito',mensajeEmpresa);
  ...
  $('#abrirConfiguracionEmpresa').addEventListener('click',()=>{
    ...
    mostrarMensaje('El Directorio oficial está integrado en esta versión y no es editable. Para obtener nuevamente la URL real, use “Limpiar conexión y cambiar empresa”.','exito',$('#mensajeConexionAvanzada'));
  ...
  $('#restaurarDirectorio')?.addEventListener('click',()=>{
    $('#urlDirectorioAvanzada').value=window.CONFIGURACION_FLOTAS.DIRECTORIO_EMPRESAS_URL||'';
    mostrarMensaje('La URL oficial ya está integrada y permanece fija.','exito',$('#mensajeConexionAvanzada'));
  });
  $('#formularioConexionAvanzada')?.addEventListener('submit',event=>{
    event.preventDefault();
    mostrarMensaje('La URL del Directorio no se modifica desde el navegador. Use la reconfiguración de empresa si necesita resolver nuevamente la URL real.','exito',$('#mensajeConexionAvanzada'));
  });
