(()=>{
  ...
  async function registrarSW(){
    try{
      ...
      const reg=await navigator.serviceWorker.register('./sw-sgf-4356.js',{scope:'./',updateViaCache:'none'});
  ...
  function prefetchEstatico(){
    link('prefetch','sgf-module.4356.js','script');
  ...
  function clavePrecarga(api){
    try{
      ...
      return `sgf_perf_4356_${e.empresa_id||u.EMPRESA_ID||'sin'}_${u.ID||'sin'}_${u.VERSION_PERMISOS||0}`;
    }catch(_){return 'sgf_perf_4356_sin';}
  }
  ...
  if(/(?:^|\/)index\.html$/.test(location.pathname)||location.pathname.endsWith('/'))link('prefetch','main.html?v=4.3.56','document');
